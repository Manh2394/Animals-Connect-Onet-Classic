public class TopAndBottomStrategy : IGameStrategy
{
    public void DoPair (int[,] matrix, Cell cell1, Cell cell2)
    {
        ...
        {
            for (int i = firstMoveRow; i < (rowNum - 2)/2; i++)
            {
                matrix[i, cell1Col] = matrix[i + 1, cell1Col];
            }
            matrix[(rowNum - 2)/2, cell1.col] = 0;            
        } else
        {
            for (int i = firstMoveRow; i > (rowNum - 2)/2; i--)
            {
            ...
            matrix[(rowNum - 2)/2 + 1, cell1.col] = 0;            
        ...
        {
            for (int i = secondMoveRow; i < (rowNum - 2)/2; i++)
            {
                matrix[i, cell2Col] = matrix[i + 1, cell2Col];
            }
            matrix[(rowNum - 2)/2, cell2.col] = 0;            
        } else
        {
            for (int i = secondMoveRow; i > (rowNum - 2)/2; i--)
            {
            ...
            matrix[(rowNum - 2)/2 + 1, cell2.col] = 0;            
